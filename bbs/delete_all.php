...
for ($i=$chk_count-1; $i>=0; $i--)
{
    ...
    */
	...
	if($bo_table == 'dental') { // 병원 삭제
		// 이벤트 삭제
		$sql_ev = " select * from g5_write_event where wr_4 = '{$write['wr_code']}' ";
		$result_ev = sql_query($sql_ev);
		$total_ev = sql_num_rows($result_ev);
		$count_ev += $total_ev;
		sql_query(" delete from g5_write_event where wr_4 = '{$write['wr_code']}' ");

		// 후기 삭제
		sql_query(" delete from {$g5['g5_shop_item_use_table']} where it_id = '{$write['wr_code']}' ");
	}
}
...
// 이벤트 글숫자 감소
if($bo_table == 'dental') {
	sql_query(" update {$g5['board_table']} set bo_count_write = bo_count_write - '$count_ev' where bo_table = 'event' ");
}
