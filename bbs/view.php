...
if ($sca || $stx || $stx === '0') {
	if ($stx && $ct) {
		$list_href = get_pretty_url($bo_table,'',$qstr.'&amp;ct='.$ct);
	} else if ($sca) {
		$list_href = get_pretty_url($bo_table,'',$qstr.'&amp;ct='.$ct);	
	} else {
		// where 문을 얻음
...
// tv 는 today view 약자
$saved_dt = false;
$tv_idx_dt = (int)get_session("ss_tv_idx_dt");
if ($tv_idx_dt > 0) {
    for ($i=1; $i<=$tv_idx_dt; $i++) {
        if (get_session("ss_tv_dt[$i]") == $wr_id) {
            $saved_dt = true;
            break;
        }
    }
...
if (!$saved_dt) {
    $tv_idx_dt++;
    set_session("ss_tv_idx_dt", $tv_idx_dt);
    set_session("ss_tv_dt[$tv_idx_dt]", $wr_id);
}
// 오늘 본 치과 저장 끝
}
...
// tv 는 today view 약자
$saved_ev = false;
$tv_idx_ev = (int)get_session("ss_tv_idx_ev");
if ($tv_idx_ev > 0) {
    for ($i=1; $i<=$tv_idx_ev; $i++) {
        if (get_session("ss_tv_ev[$i]") == $wr_id) {
            $saved_ev = true;
            break;
        }
    }
...
if (!$saved_ev) {
    $tv_idx_ev++;
    set_session("ss_tv_idx_ev", $tv_idx_ev);
    set_session("ss_tv_ev[$tv_idx_ev]", $wr_id);
}
// 오늘 본 이벤트 저장 끝
}
