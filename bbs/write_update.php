...
if($bo_table == 'request'){
	$row = " select * from g5_write_request where wr_subject = '{$_POST['wr_subject']}' and mb_id = '{$member['mb_id']}' ";
	if($row['wr_id']) alert("이미 예약된 이벤트입니다. 감사합니다.");
} else {
// 토큰체크
...
if ($w == '' || $w == 'r') {
    ...
    if ($member['mb_id']) {
        $mb_id = $member['mb_id'];
        $wr_name = addslashes(clean_xss_tags($_POST['wr_name']));
        $wr_password = '';
...
if($bo_table == 'point_wallet' && $wr_10 == 1) alert('신청이 완료되었습니다.');
if($bo_table == 'request') alert('예약이 완료되었습니다.');

if($bo_table == 'dental') {
$redirect_url = run_replace('write_update_move_url', short_url_clean(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr.'&amp;den_id='.$wr['wr_business']), $board, $wr_id, $w, $qstr, $file_upload_msg);
} else {
