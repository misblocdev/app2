...
if ($board['bo_use_category']) {
	...
	if($bo_table == 'dental' || $bo_table == 'info') {
    ...
    for ($i=0; $i<count($categories); $i++) {
		...
		if($bo_table == 'dental'){
			$category_option .= '<li><a href="'.(get_pretty_url($bo_table,'','sca='.urlencode($category).'&amp;ct=1')).'"';		
		} else {
...
$notice_count = 0;
$notice_array = array();
if($bo_table == 'event' && $den_id) {
	$den_list = " and mb_id = '{$member['mb_id']}' ";
} else if(($bo_table == 'inquiry' || $bo_table == 'request') && $den_id) {
	$den_list = " and wr_10 = '{$den_id}' ";
}
...
if (!$is_search_bbs) {
    ...
    for ($k=0; $k<$board_notice_count; $k++) {
        ...
        $row = sql_fetch(" select * from {$write_table} where wr_id = '{$arr_notice[$k]}' {$den_list} ");
...
if ($is_search_bbs) {
    $sql = " select distinct wr_parent from {$write_table} where {$sql_search} {$den_list} {$sql_order} limit {$from_record}, $page_rows ";
} else {
    $sql = " select * from {$write_table} where wr_is_comment = 0 {$den_list} ";
    if(!empty($notice_array))
...
if($page_rows > 0) {
    ...
    while ($row = sql_fetch_array($result))
    {
        ...
        if ($is_search_bbs)
            $row = sql_fetch(" select * from {$write_table} where wr_id = '{$row['wr_parent']}' {$den_list} ");
...
$sql_den = ' select * from g5_write_dental where mb_id = "'.$member['mb_id'].'" ';
$row_den = sql_fetch($sql_den);

$write_href = '';
if ($member['mb_level'] >= $board['bo_write_level']) {
	if($bo_table == 'dental' && $row_den['wr_id'] && !$is_admin) {
		$write_href = 'javascript: alert(\'치과는 한곳만 등록하실 수 있습니다.\')';
	} else {
