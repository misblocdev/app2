...
$sql_common = "  from {$g5['g5_shop_item_use_table']} a
                 left join g5_write_dental b on (a.it_id = b.wr_code)
...
// 삭제요청수
$sql = " select count(*) as cnt from {$g5['g5_shop_item_use_table']} a left join g5_write_dental b on (a.it_id = b.wr_code) left join {$g5['member_table']} c on (a.mb_id = c.mb_id) where is_delete = '1' ";
//echo $sql;
$row = sql_fetch($sql);
...
<div class="local_ov01 local_ov">
    ...
    <a href="?sfl=is_delete&amp;stx=1"><span class="btn_ov01"><span class="ov_txt"> 삭제요청</span><span class="ov_num">  <?php echo $delete_count; ?>건</span></span></a>
</div>
...
<select name="sfl" id="sfl">
    <option value="wr_subject" <?php echo get_selected($sfl, 'wr_subject'); ?>>병원명</option>
    <option value="a.it_id" <?php echo get_selected($sfl, 'a.it_id'); ?>>병원코드</option>
    <option value="is_name" <?php echo get_selected($sfl, 'is_name'); ?>>이름</option>
</select>

<label for="stx" class="sound_only">검색어<strong class="sound_only"> 필수</strong></label>
<input type="text" name="stx" id="stx" value="<?php echo $sfl != 'is_delete' ? $stx : ''; ?>" required class="frm_input required">
...
<div class="tbl_head01 tbl_wrap" id="itemuselist">
    ...
    <tr>
        ...
        <th scope="col"><?php echo subject_sort_link("wr_subject"); ?>병원명</a></th>
        ...
        <th scope="col"><?php echo subject_sort_link("is_delete"); ?>삭제요청</a></th>
        <th scope="col">관리</th>
    </tr>
    </thead>
    <tbody>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $href = get_pretty_url('dental', $row['wr_id']).'&den_id='.$row['wr_code'];
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td class="td_left"><a href="<?php echo $href; ?>"><?php echo $row['wr_subject']; ?></a></td>
        ...
        <td class="td_left">
            ...
            <a href="#" class="delete_href" onclick="return false;" target="<?php echo $i; ?>" style="color: red;">삭제 요청 사유<span class="tit_op">열기</span></a>
            <div id="delete_div<?php echo $i; ?>" class="delete_div" style="display:none;">
                <?php echo $row['is_delete_content']; ?>
            </div>            
			<?php }?>
        </td>
...
$(function(){
    ...
    $(".delete_href").click(function(){
        var $content = $("#delete_div"+$(this).attr("target"));
        $(".delete_div").each(function(index, value){
            if ($(this).get(0) == $content.get(0)) { // 객체의 비교시 .get(0) 를 사용한다.
                $(this).is(":hidden") ? $(this).show() : $(this).hide();
            } else {
                $(this).hide();
