...
$den_id = isset($_REQUEST['den_id']) ? get_search_string(trim($_REQUEST['den_id'])) : '';
$is_id  = isset($_REQUEST['is_id']) ? preg_replace('/[^0-9]/', '', trim($_REQUEST['is_id'])) : 0;
$use = array('is_subject'=>'', 'is_content'=>'');

if (G5_IS_MOBILE) {
    include_once(G5_MOBILE_PATH.'/reviewform.php');
    return;
}
...
if (!$is_member) {
    alert_close("진료후기는 회원만 작성 가능합니다.");
}
...
$reviewform_skin = G5_REVIEW_SKIN_PATH.'/reviewform.skin.php';

if(!file_exists($reviewform_skin)) {
    echo str_replace(G5_PATH.'/', '', $reviewform_skin).' 스킨 파일이 존재하지 않습니다.';
} else {
    include_once($reviewform_skin);
}
