...
<section id="bo_w" style="font-size: 15px;">
    ...
    <input type="hidden" name="wr_4" value="<?php echo $member['mb_3'] ?>">
	<?php
	$sql_dt = ' select * from g5_write_dental where mb_id = "'.$member['mb_id'].'" ';
	$row_dt = sql_fetch($sql_dt);

	if($row_dt['wr_code']){
		$addr = explode(' ', $row_dt['wr_2']);
	} else {
		$addr = explode(' ', $member['mb_addr1']);
	}
	?>
    <input type="hidden" name="wr_3" value="<?php echo $addr[0] ?>">
    ...
    <div class="jtbl_frm01 tbl_wrap">
		...
		<tr class="write_div">
			<th scope="row"><label for="wr_1">시작날짜</label></th>
			<td><input type="text" name="wr_1" value="<?php echo $wr_1 ?>" id="datepicker" class="frm_input" size="10"></td>
		</tr>
		<tr class="write_div">
			<th scope="row"><label for="wr_2">종료날짜</label></th>
			<td><input type="text" name="wr_2" value="<?php echo $wr_2 ?>" id="datepicker2" class="frm_input" size="10"></td>
		</tr>
