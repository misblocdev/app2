...
// 게시글리스트 썸네일 생성
function get_list_thumbnail2($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=false, $option=0, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    global $g5, $config;
    $filename = $alt = "";
    $edt = false;
	...
	$bfno ="";
	if($option){
		$bfno = "and bf_no = '$option'";
	}


    $sql = " select bf_file, bf_content from {$g5['board_file_table']}
                where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_type between '1' and '3' {$bfno} order by bf_no limit 0, 1 ";
    $row = sql_fetch($sql);
