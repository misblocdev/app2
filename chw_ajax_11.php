...
$sql1 = sql_fetch("select * from basic_set where mb_id='".$member['mb_id']."'");
$sql2 = sql_query("select * from basic_set2 where mb_id='".$member['mb_id']."'");
$sql2num = sql_num_rows($sql2);
...
if($sql2num>0){
	for($i=0;$row=sql_fetch_array($sql2);$i++){
		...
		$wr_date = $wr_date."|".$row['wr_date'];
		}

		if($row['wr_times']){ // 등록된 시간이 있으면
...
var okyoil = "/"+"<?php echo $sql1['yoil'];?>"+"/";

var wrdate = "<?php echo $wr_date;?>";
...
{	
	...
	if(dnow>i && ddd==0){ // 이번달 오늘 이전
		...
		/*
		if(okyoil.match(yoilname)){	
		htmld +='<td>'+'<span class="chch">'+i+'</span>'+'</td>';			
		}else{
		htmld +='<td>'+'<span class="chch">'+'휴무'+'</span>'+'</td>';
		}*/

		if(okyoil.match(yoilname)){ // 기본 세팅에 선택된 요일인지
			if(wrdate.match(chw_date+chw_d)){
				if(div1[i-1]){ // 날짜 개별설정된 시간이 있는지
					htmld +='<td>'+'<span class="chch">'+'시간'+'</span>'+'</td>';
				}else{ // 날짜 개별설정 휴무
					htmld +='<td>'+'<span class="chch">'+'휴무'+'</span>'+'</td>';
				}
			}else{
				htmld +='<td>'+'<span class="chch">'+i+'</span>'+'</td>';
				//htmld +='<td style="position:relative;">'+'<a href="javascript:;" data-date="'+chw_date+chw_d+'" class="date_click">'+i+'</a>'+'<span class="spanjum"></span>'+'</td>';
			}
		}else{ // 기본 세팅에 선택 안된 요일은 휴무
			htmld +='<td>'+'<span class="chch">'+'휴무'+'</span>'+'</td>';
		}	
	}else if(ddd<0){ // 이전달
		...
		if(wrdate.match(chw_date+chw_d)){
			if(div1[i-1]){
			htmld +='<td style="position:relative;">'+'<a href="javascript:;" data-date="'+chw_date+chw_d+'" class="date_click">'+'시간'+'</a>'+'<span class="spanjum"></span>'+'</td>';
			}else{
			htmld +='<td style="position:relative;">'+'<a href="javascript:;" data-date="'+chw_date+chw_d+'" class="date_click">'+'휴무'+'</a>'+'<span class="spanjum"></span>'+'</td>';
			}
			}else{
